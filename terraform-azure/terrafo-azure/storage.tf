...
resource "azurerm_storage_account_network_rules" "sa" {
    ...
    virtual_network_subnet_ids = [
        for subnet_id in azurerm_subnet.vnet_subnets : subnet_id.id
    ]
}
...
# This block of code creates a resource of type "azurerm_storage_share", which represents an Azure Storage Share
resource "azurem_private_dns_zone" "storage_share_private_zone" {
    name = "privatelink.file.core.windows.net"
    resource_group_name = azurerm_resource_group_name.name
    tags = var.default_tags
...
resource "azurerm_private_dns_zone_virtual_network_link" "storage_share_private_zone" {
    ...
    resource_group_name = azurerm_resource_group.resource_group.name
    private_dns_zone_name = azurerm_private_dns_zone.storage_storage_share_private_zone.name
